...
class sale_order(osv.osv):
    ...
    _columns = {
       ...
       'discount_amt': fields.function(
            _calculate_discount, string='- Discount', digits_compute=dp.get_precision('Account'),readonly=True, store=True
        ),
        ...
        'amount_total': fields.function(_amount_all_wrapper, digits_compute=dp.get_precision('Account'), string='Total',
            store={
                'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line', 'discount_method', 'discount_amount', 'discount_amt'], 10),
                'sale.order.line': (_get_order, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
            multi='sums', help="The total amount."),
        'visible_report' : fields.boolean(string = "Visible on Report"),
    }
